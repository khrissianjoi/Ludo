...
class TokenCreate:
    ...
    def drawOtherPlayersTokens(self, otherPlayers, refresh):
        for player in otherPlayers:
            for token in player.tokensOnPath:
                new_translated_token_path = [[x + token.tokenLocation[0][0], token.tokenLocation[1][0] + y] for [x, y] in self.tokenPoly]
                pygame.draw.polygon(refresh.gameDisplay, player.colour, new_translated_token_path)
                pygame.draw.polygon(refresh.gameDisplay,  self.colours.BLACK, new_translated_token_path, 1)
            # when other player is doing their turn all of the other player's tokens don't move
            for token in player.tokensOnBase:
                new_translated_token_path = [[x + token.xBaseCoord, token.yBaseCoord+y] for [x, y] in self.tokenPoly]
                pygame.draw.polygon(refresh.gameDisplay, player.colour, new_translated_token_path)
                pygame.draw.polygon(refresh.gameDisplay,  self.colours.BLACK, new_translated_token_path, 1)
            for token in player.tokensOnHome:
                new_translated_token_path = [[x*0.85 + token.xHomeCoord, token.yHomeCoord + y*0.85] for [x, y] in self.tokenPoly]
                pygame.draw.polygon(refresh.gameDisplay, player.colour, new_translated_token_path)
                pygame.draw.polygon(refresh.gameDisplay, self.colours.BLACK, new_translated_token_path, 1)
    ...
    def moveOneToken(self, refresh, colour, path):
        ...
        pygame.draw.polygon(refresh.gameDisplay, self.colours.BLACK,  path, 1)
    ...
    def drawTokenOnHome(self, refresh, currentPos, moveBy, otherPlayers):
        ...
        translated_token_path = [[x*0.85 + self.xHomeCoord, y*0.85 + self.yHomeCoord] for [x, y] in self.tokenPoly]
        pygame.draw.polygon(refresh.gameDisplay, self.tokenID[1], translated_token_path)
        pygame.draw.polygon(refresh.gameDisplay, self.colours.BLACK, translated_token_path, 1)
        self.playerOwner.drawTokens(refresh, self, self.tokenPoly)
        self.drawOtherPlayersTokens(otherPlayers, refresh)
    ...
    def drawToken(self, refresh):
        if self.tokenTilesPath[self.currentTilePathPosition] == 0:
            new_translated_token_path = [[x + self.tokenLocation[0][0], self.tokenLocation[1][0] + y] for [x, y] in self.tokenPoly]
